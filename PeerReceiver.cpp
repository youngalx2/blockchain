	...
	while(true) {
		...
		while (strcmp(cmd, "disconnect")) {
			if (!strcmp(cmd, "ls")) {
				...
				FileReaderAndReceiver(fileName.c_str(), newsockfd);
			}
...
void PeerReceiver::FileReaderAndReceiver(const char* file_name, int socket_id) {

	char buff[MAX_BUFFER_LEN + 3];
	memset(buff, 0, sizeof(buff));
    char folder[sizeof(SERVER_SHARED_DIR)+1];
    strcpy(folder, SERVER_SHARED_DIR);
	FILE *fp = fopen(strcat(strcat(folder, "/"), file_name), "rb");
	if (fp == NULL) {
